...
if(isset($_POST['submit'])){
   ...
   $correo_adm = " SELECT * FROM administrador WHERE email = '$email'";
   $contra_adm = " SELECT * FROM administrador WHERE password = '$pass' ";

   $result = mysqli_query($conn, $select);
   $result_correo_cliente = mysqli_query($conn, $correo_cliente);
   $result_contra_cliente  = mysqli_query($conn, $contra_cliente);
   $result_correo_adm = mysqli_query($conn, $correo_adm);
   $result_contra_adm = mysqli_query($conn, $contra_adm);
   ...
   if(mysqli_num_rows($result) > 0){
      ...
      if($row['user_type'] == 'admin'){
         ...
         $_SESSION['admin_name'] = $row['nombre_cliente'];
      ...
      }else if ((!mysqli_num_rows($result_correo_cliente) > 0) and (!mysqli_num_rows($result_correo_adm) > 0)){
      ...
      }else if((!mysqli_num_rows($result_contra_cliente) > 0) and (!mysqli_num_rows($result_contra_adm) > 0)){
